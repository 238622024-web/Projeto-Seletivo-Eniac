...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $stmt = $pdo->prepare("
            INSERT INTO vagas (titulo, empresa, localizacao, salario_min, salario_max, tipo_contrato, modalidade, vagas_disponiveis, data_encerramento, status, descricao, requisitos, beneficios, palavras_chave, data_publicacao, data_criacao) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURDATE(), NOW())
        ");
        $palavras_chave = $_POST['palavras_chave'] ?? '';
        $stmt->execute([
            $titulo, $empresa, $localizacao, $salario_min, $salario_max,
            $tipo, $modalidade, $vagas_disponiveis, $data_encerramento ?: null, $status, $descricao, $requisitos, $beneficios, $palavras_chave
        ]);
            ...
            <form method="POST" action="" class="space-y-8 animate-slide-up">
                ...
                <div class="bg-white rounded-xl shadow-lg p-6 card-hover transition-all duration-300 animate-bounce-in" style="animation-delay: 0.4s;">
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="space-y-2">
                            <label for="palavras_chave" class="block text-sm font-semibold text-gray-700">
                                <i class="fas fa-key mr-2 text-gray-400"></i>
                                Palavras-chave da vaga (IA)
                            </label>
                            <input type="text" id="palavras_chave" name="palavras_chave"
                                   class="w-full px-4 py-3 border border-blue-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent input-focus transition-all duration-300"
                                   placeholder="Ex: marketing, digital, social media, tráfego, conteúdo, anúncios"
                                   value="<?php echo htmlspecialchars($_POST['palavras_chave'] ?? ''); ?>">
                            <span class="text-xs text-gray-500">Separe por vírgula. Essas palavras serão usadas pela IA para comparar com os currículos.</span>
                        </div>
                        </div>
